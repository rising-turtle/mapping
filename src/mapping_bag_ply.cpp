...
int skip_front_k_frames = 60; // skip data in the first kth frames
int main(int argc, char* argv[])
{
  ...
  cout<<"usage: ./mapping_bag_ply [traj] [bag] [out_ply] [step]"<<endl;
...
void processBagFile(string bagfile, vector<trajItem>& v_traj, vector<float>& pts_loc, vector<unsigned char>& pts_col, int skip, int* rect)
{
  ...
  BOOST_FOREACH(rosbag::MessageInstance const m, view){
    ...
    if(m.getTopic() == rgb_tpc){
      ...
      // if(n_cnt < 50)
      printf("mapping_bag_ply.cpp: handle %d dpt frames at %ld \n", n_cnt, timestamp);
      }
